...
> **Dementia-Specific Supplement:** This document defines the baseline requirements for the Elderly AI Assistant. For requirements specific to users with early-onset and moderate dementia — including double-dose prevention, inactivity monitoring, repetition detection, caregiver override, orientation support, and dementia-adapted voice interaction — see [`requirements-dementia-supplement.md`](requirements-dementia-supplement.md). The supplement takes precedence where the two documents conflict on dementia-specific concerns.
