...
final class LlamaCommandInterpreter: CommandInterpreter {
    ...
    private func runInference(prompt: String,
        ...
        // The `Template` given to `LLM(from:)` is only applied by
        // `respond(to:)`, which this class never calls — so the chat
        // framing is applied manually here, with LLaMA 3.2's official
        // scheme (system/user/assistant headers + <|eot_id|> ends). The
        // runtime's `generateWithConstraints` consumes the RAW string with
        // no template preprocessing, exactly like the old
        // `getCompletion(from:)` did, and breaks on the eos/eot tokens the
        // model emits after the JSON object.
