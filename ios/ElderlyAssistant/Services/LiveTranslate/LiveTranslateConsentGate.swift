...
// C09 — `LiveTranslateConsentGate` (T-014).
//
// The feature's compliance basis: no recognized text leaves the device
// without a recorded grant for the disclosure copy that was shown
// (FR-LCT-010). This file exists to make five properties structural rather
// than aspirational:
//
...
//    owner action (a bug to fix) from "the elder has not decided"
//    (NFR-LCT-007, T-002's taxonomy). Absent, corrupt, stale-version and
//    scope-mismatched records are all non-granted states; there is no
//    default-on path and no configuration value anywhere in this file that
//    can produce a grant.
...
//    control (T-015); no configuration, family-setting or remote path has a
//    reason to call either, and none does.
...
//    (T-018, AM-7) can require the proof as a parameter and a caller that
//    forgot to consult the gate is a compile error rather than a review
//    finding.
...
//  - **Withdrawal is immediate and total (FR-LCT-012, AM-1).** `revoke()`
//    flips the in-memory deny first, then cancels every registered
//    in-flight request, then deletes the record and verifies the delete by
//    reading back. The tier re-reads `authorize()` before every attempt
...
final class LiveTranslateConsentGate {
    ...
    enum ConsentError: Error, Equatable, LogSafeErrorCode {
        ...
        var logSafeErrorCode: String {
            switch self {
            case .recordUnreadable: return "consent_record_unreadable"
            case .writeFailed: return "consent_write_failed"
            }
        }
    }
    ...
    /// `authorize()` is the only way one can exist. A builder that takes a
    /// `Grant` therefore
    /// cannot be reached without passing through the gate; a missing check
    /// is a compile error, not a remembered discipline.
    ///
    /// A `Grant` is a statement about the read that produced it, not a
    /// standing permission: the tier mints one per attempt (AM-1), and a
    /// revocation cancels whatever is in flight through
    /// `registerInFlight(_:)`.
    ...
    static let storageKey = "plugin.live_translate.consent.v1"
    ...
    /// Cancellation hooks for in-flight tier-2 work, keyed by registration.
    ...
    /// (grant or decline) and from the revocation control (T-015); no
    /// configuration path calls it.
    ///
    ...
    // MARK: - Revocation (AM-1, AM-4, FR-LCT-012)
    ...
    ///    retry window, together with the tier's per-attempt re-read).
    ...
    func revoke() -> Result<Void, ConsentError> {
        ...
        // 2. Delete, then verify — the read-back is the authority. A
        //    `delete` that reports failure while the record is genuinely
        //    gone has still revoked; a `delete` that reports success while
        //    the record survives has not.
        withLock { _ = storage.delete(key: Self.storageKey) }

        // 3. If a granted record survived the delete, overwrite it with a
        //    deny record through the one write path, then verify again. This
        //    is the step that stops a relaunch from re-granting from a record
        //    the elder believes they withdrew (AM-4 part 4).
        //
        //    An *unverifiable* read-back takes the same route: "the delete
        //    returned success" is the store's claim, and a claim that cannot
        //    be checked is not a revocation that can be reported as done. The
        //    tombstone is attempted; whether it lands, the withdrawal is
        //    reported as a failure unless a read-back confirms it.
        ...
        //    of this process; the caller gets a failure, never a success, and
        //    the elder is told the withdrawal did not take effect.
        switch grantEvidence() {
        case .noGrant:
            events.consentRevoked()
            return .success(())
    ...
    private func grantEvidence() -> GrantEvidence {
        withLock {
            ...
            case .failure:
                ...
                // a later launch (the file-protection state a locked device
                // produces is exactly this shape), so it is never counted as
    ...
    /// Registers in-flight tier-2 work so a revocation can cancel it. The
    /// tier holds the returned registration for the duration of an attempt
    /// and releases it on every exit path; a revocation cancels whatever is
    ...
    /// and for `security-test`; the tier is the only thing that registers.
