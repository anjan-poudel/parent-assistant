...
open class LLM: ObservableObject {
    ...
    public init?(
        ...
        #else
        // elderly-ai-assistant (2026-09-06): llama_model_default_params()
        // leaves n_gpu_layers at 0 = full CPU even though the bundled
        // XCFramework ships Metal. Offload every layer on device — the
        // 1B/3B Q4_K_M intent models fit comfortably in the RAM tiers the
        // catalog already gates (3 GB+), and ggml falls back to CPU if
        // Metal init fails. (Upstream eastriverlee/LLM.swift pins main;
        // this is why we vendored.)
