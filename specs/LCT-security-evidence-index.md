...
| `LiveTranslationPipelineTests.testScenarioOneOutageIsOneDegradationEventWhicheverLayerReportsIt` | **Which layer emits `translation_degraded`** (the ownership the review of d33089d required): the cloud tier reports the failures it *returns* — it is the layer that knows the failure class, that counts the regions a failure covers, and the only emission that survives a caller **reserving** a string instead of settling it (the reliability router's shape, where the string is answered on the device and the outage would otherwise leave the evidence entirely). The pipeline's terminal writer reports every degradation it wrote itself — a gate refusal no tier ever saw, a device ending, a held frame's own degradation — and skips exactly those that arrived in a `BatchResult`. Both halves are asserted: `1` event with `regionCount = 1` for a settled tier failure, and `1` event with `reason = consent_not_granted` for a refusal. Suppressing either side is the failure this row exists to catch — deleting the tier's emission took the evidence with it for every failure the caller never settles |
